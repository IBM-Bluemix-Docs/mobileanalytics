...
您在 {{site.data.keyword.mobileanalytics_short}} 主控台中看到的度量值只是您能從行動資料蒐集到的見解範例。您可以自動將行動資料以管道傳到 {{site.data.keyword.IBM}} Db2 Warehouse，您可以在這裡自訂分析、彙總資料與其他公用和專用資料來源，以及套用頂尖分析來衍生深入、詳細且準確的見解，協助您瞭解和推動業務。
